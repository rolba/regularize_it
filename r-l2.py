"""
This is an experiment with different alpha dropout layers placeholding in model.
...
from tensorflow.keras.layers import Dense, Conv2D, Flatten, MaxPooling2D, SpatialDropout2D, Dropout
...
model.add(Conv2D(64, kernel_size=(3, 3), activation=tensorflow.nn.relu, input_shape=input_shape))
model.add(SpatialDropout2D(0.2))
model.add(Conv2D(64, kernel_size=(3, 3), activation=tensorflow.nn.relu, kernel_regularizer=l2(lr_reg)))
model.add(SpatialDropout2D(0.2))
model.add(MaxPooling2D(pool_size=(2, 2)))

model.add(Conv2D(128, kernel_size=(3, 3), activation=tensorflow.nn.relu, kernel_regularizer=l2(lr_reg)))
model.add(SpatialDropout2D(0.5))
model.add(Conv2D(128, kernel_size=(3, 3), activation=tensorflow.nn.relu, kernel_regularizer=l2(lr_reg)))
model.add(SpatialDropout2D(0.5))
model.add(MaxPooling2D(pool_size=(2, 2)))
...
# opt = tensorflow.keras.optimizers.SGD(learningrate, momentum=0.9, decay=learningrate / epochs, nesterov=True)
